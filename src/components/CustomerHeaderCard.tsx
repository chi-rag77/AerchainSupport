...
const CustomerHeaderCard = ({ customerName, tickets }: CustomerHeaderCardProps) => {
  const metrics = useMemo(() => {
    if (!tickets || tickets.length === 0) {
      return {
        ...
        slaAdherencePct: "N/A",
        reopenRatePct: "N/A", // Not feasible with current data
        escalationCount: 0,
        openTicketsCount: 0,
        openTicketsBreakdown: { high: 0, medium: 0, low: 0, urgent: 0, other: 0 },
      };
    }
    ...
    const slaAdherencePct = slaApplicableCount > 0
      ? `${((slaMetCount / slaApplicableCount) * 100).toFixed(1)}%`
      : "N/A";

    const escalationCount = tickets.filter(t => t.status.toLowerCase() === 'escalated').length;
    ...
    const openTicketsBreakdown = { high: 0, medium: 0, low: 0, urgent: 0, other: 0 };
    openTickets.forEach(ticket => {
      const priorityLower = ticket.priority.toLowerCase();
      if (priorityLower === 'high') openTicketsBreakdown.high++;
      else if (priorityLower === 'medium') openTicketsBreakdown.medium++;
      else if (priorityLower === 'low') openTicketsBreakdown.low++;
      else if (priorityLower === 'urgent') openTicketsBreakdown.urgent++;
    ...
    return {
      ...
      avgResolutionTime,
      slaAdherencePct,
      reopenRatePct: "N/A", // Requires historical status changes
      escalationCount,
      openTicketsCount,
  ...
  // Placeholder for Customer Tier
  const customerTier = "Gold"; // This would come from a custom field or external data
    ...
    <Card className="relative overflow-hidden group transition-all duration-300 hover:shadow-lg hover:scale-[1.02] h-full">
      ...
      <CardContent className="text-sm space-y-3">
        <div className="grid grid-cols-2 gap-x-4 gap-y-2">
          ...
          <div className="flex flex-col">
            <span className="text-muted-foreground flex items-center gap-1"><Clock className="h-4 w-4" /> Avg. Resolution Time:</span>
            <span className="text-xl font-bold text-foreground">{metrics.avgResolutionTime}</span>
          </div>
          <div className="flex flex-col">
            ...
            <span className="text-xl font-bold text-foreground">{metrics.slaAdherencePct}</span>
          </div>
          <div className="flex flex-col">
            <span className="text-muted-foreground flex items-center gap-1"><AlertCircle className="h-4 w-4 text-red-500" /> Escalation Count:</span>
            <span className="text-xl font-bold text-foreground">{metrics.escalationCount}</span>
          </div>
          <div className="flex flex-col">
        ...
        <div className="mt-4 pt-3 border-t border-border text-xs text-muted-foreground">
          <p className="flex items-center"><Users className="h-3 w-3 inline-block mr-1 text-red-500" /> Reopen Rate: {metrics.reopenRatePct} (Requires historical status changes)</p>
        </div>
      </CardContent>
    </Card>
  );
