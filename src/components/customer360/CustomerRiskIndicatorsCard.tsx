...
import { AlertTriangle, Clock, CalendarX, ListOrdered, ArrowRight } from 'lucide-react';
...
interface CustomerRiskIndicatorsCardProps {
  customerName: string;
  tickets: Ticket[];
  onViewTicketDetails: (ticket: Ticket) => void; // Callback to view ticket details
}
...
const CustomerRiskIndicatorsCard = ({ customerName, tickets, onViewTicketDetails }: CustomerRiskIndicatorsCardProps) => {
  const riskMetrics = useMemo(() => {
    ...
    const longOpenTicketsCount = openTickets.filter(t => differenceInDays(now, parseISO(t.created_at)) >= 7).length; // Tickets open for 7+ days

    const oldestOpenTickets = openTickets
      .sort((a, b) => parseISO(a.created_at).getTime() - parseISO(b.created_at).getTime())
      .slice(0, 5); // Top 5 oldest open tickets
      ...
      <CardContent className="text-sm space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="flex flex-col p-3 bg-gray-50 dark:bg-gray-700 rounded-lg shadow-sm">
            ...
            <span className="text-2xl font-bold text-red-500">{riskMetrics.overdueTicketsCount}</span>
          </div>
          ...
          <div className="flex flex-col p-3 bg-gray-50 dark:bg-gray-700 rounded-lg shadow-sm">
            ...
            <span className="text-2xl font-bold text-orange-500">{riskMetrics.longOpenTicketsCount}</span>
          </div>
        </div>
        ...
        {riskMetrics.oldestOpenTickets.length > 0 && (
          <div className="mt-4 pt-3 border-t border-border">
            <h4 className="font-semibold text-foreground mb-2 flex items-center gap-2">
              <ListOrdered className="h-4 w-4 text-muted-foreground" /> Oldest Open Tickets:
            </h4>
            <ul className="space-y-2">
              {riskMetrics.oldestOpenTickets.map(ticket => (
                <li key={ticket.id} className="flex items-center justify-between p-2 bg-white dark:bg-gray-800 rounded-md border border-border shadow-sm">
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <span className="flex-grow truncate text-foreground font-medium">
                        {ticket.subject}
                      </span>
                    </TooltipTrigger>
