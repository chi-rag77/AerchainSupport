...
interface CustomerTicketTypeChartProps {
  tickets: Ticket[];
}

const TYPE_COLORS: { [key: string]: string } = {
  bug: "hsl(10 80% 70%)", // Red
  csTask: "hsl(28 100% 70%)", // Orange
  duplicate: "hsl(48 100% 70%)", // Yellow
  notRelevant: "hsl(120 60% 70%)", // Green
  query: "hsl(180 60% 70%)", // Cyan
  techTask: "hsl(240 60% 70%)", // Purple
  'Unknown Type': "hsl(210 10% 70%)", // Gray for unknown
};
...
const CustomerTicketTypeChart = ({ tickets }: CustomerTicketTypeChartProps) => {
  const processedData = useMemo(() => {
    ...
    // Sort types for consistent color mapping and display
    const sortedTypes = Array.from(typeCounts.keys()).sort((a, b) => typeCounts.get(b)! - typeCounts.get(a)!);

    return sortedTypes.map(type => ({
      name: type,
      value: typeCounts.get(type)!,
      color: TYPE_COLORS[type.toLowerCase()] || TYPE_COLORS['Unknown Type'],
    }));
  }, [tickets]);
...
export default CustomerTicketTypeChart;