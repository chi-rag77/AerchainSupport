...
const AITrendCard = ({ intelligence, signals, isLoading }: AITrendCardProps) => {
  if (isLoading) {
    return (
      <Card className="h-full flex flex-col items-center justify-center p-8 text-muted-foreground border-none bg-gray-50/50 dark:bg-gray-900/50 rounded-[24px]">
  ...
  // If we have signals but no AI intelligence yet, show a partial state
  const riskLevel = signals?.riskLevel || 'LOW';
  const riskConfig = {
  ...
  return (
    <Card className={cn("h-full flex flex-col border-2 shadow-sm rounded-[24px] overflow-hidden transition-all duration-500", riskConfig.bg, riskConfig.border)}>
      <CardHeader className="p-6 pb-4">
        ...
        <CardTitle className="text-lg font-bold flex items-center gap-2">
          Risk Level: <span className={riskConfig.color}>{riskLevel}</span>
      ...
      <CardContent className="p-6 pt-0 space-y-6 flex-grow">
        {!intelligence ? (
          <div className="flex flex-col items-center justify-center py-10 text-center space-y-2">
            ...
            <p className="text-xs text-muted-foreground">Generating deep insights...</p>
          </div>
        ) : (
          <>
