...
const Analytics = () => {
  ...
  const filteredTickets = useMemo(() => {
    ...
    // Assuming 'channel' field exists or can be derived
    // if (selectedChannels.length > 0) {
    //   currentTickets = currentTickets.filter(ticket => ticket.channel && selectedChannels.includes(ticket.channel));
    // }
  ...
  // Placeholder for unique channels - assuming a 'channel' field might exist
  const uniqueChannels = useMemo(() => {
    // In a real scenario, you'd extract this from ticket data
                    ...
                    <Calendar
                      ...
                      onSelect={(range) => {
                        if (range?.from) {
                          ...
                          setDateRangeFilter(range); // Update main filter state
                        } else {
                          setDateRangeFilter("last30days"); // Fallback
                          setCustomDateRange(undefined);
                        }
                      }}
                      numberOfMonths={2}
