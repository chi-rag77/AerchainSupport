...
interface Ticket {
  ...
  created_at: string;
  cf_company?: string;
  priority: string; // Added priority for potential future use in digest keys
}

// Define the Notification type (should match your client-side types/index.ts)
interface Notification {
  id?: string; // ID is generated by DB
  user_id: string;
  message: string;
...
serve(async (req) => {
  ...
  try {
    ...
    const { data: tickets, error: fetchError } = await supabase
      ...
      .select('freshdesk_id, subject, status, updated_at, created_at, cf_company, priority') // Added priority
      .limit(10000);
    ...
    (tickets as Ticket[]).forEach(ticket => {
      ...
      if (activeStatuses.includes(ticket.status.toLowerCase()) && daysSinceUpdate >= stalledThresholdDays) {
        ...
        const digestMessage = `Tickets stalled for ${daysSinceUpdate} days`; // Generalized message for digest
        insights.push({
        ...
        if (severity === 'critical' || severity === 'warning') {
          notificationsToProcess.push({
            ...
            digest_key: `stalledOnTech-${severity}-${digestMessage.replace(/\s/g, '-')}`, // Unique digest key
            read: false,
          });
        }
      }
    ...
    customerTicketCounts.forEach((count, companyName) => {
      if (count >= highVolumeThreshold) {
        ...
        const digestMessage = `High volume customer activity for ${companyName}`; // Generalized message for digest
        insights.push({
        ...
        if (severity === 'critical' || severity === 'warning') {
          notificationsToProcess.push({
            ...
            digest_key: `highVolumeCustomer-${severity}-${digestMessage.replace(/\s/g, '-')}`, // Unique digest key
            read: false,
          });
        }
      }
    ...
    // --- Deduplication and Insertion of Notifications ---
    const notificationsToInsert: Notification[] = [];
    for (const notification of notificationsToProcess) {
      // Check for existing similar notification within the last 24 hours
      const { data: existingNotifications, error: checkError } = await supabase
        .from('user_notifications')
        .select('id')
        .eq('user_id', notification.user_id)
        .eq('digest_key', notification.digest_key) // Use digest_key for deduplication
        .gte('created_at', twentyFourHoursAgo.toISOString());

      if (checkError) {
        console.error('Error checking for existing notifications:', checkError);
        // Continue to insert to avoid blocking, but log the error
        notificationsToInsert.push(notification);
      } else if (!existingNotifications || existingNotifications.length === 0) {
        // No similar notification found, so insert this one
        notificationsToInsert.push(notification);
      } else {
        console.log(`Skipping duplicate notification for digest_key: ${notification.digest_key}`);
      }
    }
    ...
    if (notificationsToInsert.length > 0) {
      ...
      if (notificationError) {
        ...
        console.log(`Inserted ${notificationsToInsert.length} new notifications from insights.`);
      }
    }
